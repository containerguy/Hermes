  ...
  <action>
    ...
    4. `runBulkUserImport(context, { adminId, rows, dryRun, defaultNotificationsEnabled })` (per D-08, D-09, D-10):
       ...
       g. If `dryRun === false` AND all rows are clean: open ONE `context.sqlite.transaction(() => { ... })()` and for each accepted row:
          ...
          - Catch unique-constraint errors inside the transaction: THROW to force rollback (all-or-nothing per D-08). Before re-throwing, record the offending row identity in a scoped closure variable declared in the outer handler: `let dbFailure: { username: string; reason: "db_unique" | "db_other"; errorMessage: string } | null = null`. Populate `dbFailure = { username: row.username, reason: err.code === "SQLITE_CONSTRAINT_UNIQUE" ? "db_unique" : "db_other", errorMessage: String(err?.message ?? err) }` INSIDE the inner `for` loop's catch, then `throw err` to force rollback. After the outer `try { transaction(...) }` catches that throw, map the outcome: for every row whose `row.username === dbFailure?.username` replace its pre-tx outcome with `{ status: "failed", reason: "db", message: dbFailure.errorMessage }` (at most one row is tagged); all other rows retain their validation/duplicate outcomes but `totals.created = 0` per all-or-nothing. If `dbFailure === null` (non-constraint throw, e.g., disk full), fall back to marking EVERY otherwise-clean row as `{ status: "failed", reason: "db", message: "Datenbankfehler während des Imports." }`.
